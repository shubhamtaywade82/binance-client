...
RSpec.describe BinanceUSDM::Resources::Account do
  ...
  describe "#info" do
    it "returns account information" do
      stub_request(:get, /#{base_url}\/fapi\/v2\/account/)
        .to_return(
          status: 200,
          body: {
  ...
  describe "#balance" do
    it "returns account balances" do
      stub_request(:get, /#{base_url}\/fapi\/v2\/balance/)
        .to_return(
          status: 200,
  ...
  describe "#positions" do
    it "returns positions" do
      stub_request(:get, /#{base_url}\/fapi\/v2\/positionRisk/)
        .to_return(
          status: 200,
          body: [{ "symbol" => "BTCUSDT", "positionAmt" => "0.5", "entryPrice" => "50000.00", "leverage" => "10" }].to_json,
          headers: { "Content-Type" => "application/json" }
        )
  ...
  describe "#position_mode" do
    it "returns current position mode" do
      stub_request(:get, /#{base_url}\/fapi\/v1\/positionSide\/dual/)
  ...
  describe "#change_position_mode" do
    it "changes position mode" do
      stub_request(:post, /#{base_url}\/fapi\/v1\/positionSide\/dual/)
        .to_return(status: 200, body: { "code" => 200, "msg" => "success" }.to_json, headers: { "Content-Type" => "application/json" })
  ...
  describe "#change_leverage" do
    it "changes leverage for a symbol" do
      stub_request(:post, /#{base_url}\/fapi\/v1\/leverage/)
        .to_return(
          status: 200,
  ...
  describe "#change_margin_mode" do
    it "changes margin mode for a symbol" do
      stub_request(:post, /#{base_url}\/fapi\/v1\/marginType/)
        .to_return(status: 200, body: { "code" => 200, "msg" => "success" }.to_json, headers: { "Content-Type" => "application/json" })
  ...
  describe "#modify_position_margin" do
    it "modifies isolated position margin" do
      stub_request(:post, /#{base_url}\/fapi\/v1\/positionMargin/)
        .to_return(status: 200, body: { "amount" => "100", "type" => 1 }.to_json, headers: { "Content-Type" => "application/json" })
  ...
  describe "#position_margin_history" do
    it "returns position margin change history" do
      stub_request(:get, /#{base_url}\/fapi\/v1\/positionMargin\/history/)
        .to_return(status: 200, body: [{ "symbol" => "BTCUSDT", "amount" => "100", "type" => 1 }].to_json, headers: { "Content-Type" => "application/json" })
  ...
  describe "#income_history" do
    it "returns income history" do
      stub_request(:get, /#{base_url}\/fapi\/v1\/income/)
        .to_return(
          status: 200,
  ...
  describe "#commission_rate" do
    it "returns commission rate for symbol" do
      stub_request(:get, /#{base_url}\/fapi\/v1\/commissionRate/)
        .to_return(
          status: 200,
  ...
  describe "listenKey management" do
    it "creates, keeps alive, and closes listenKey" do
      stub_request(:post, /#{base_url}\/fapi\/v1\/listenKey/)
        .to_return(status: 200, body: { "listenKey" => "test_key_123" }.to_json, headers: { "Content-Type" => "application/json" })
      stub_request(:put, /#{base_url}\/fapi\/v1\/listenKey/)
        .to_return(status: 200, body: {}.to_json, headers: { "Content-Type" => "application/json" })
      stub_request(:delete, /#{base_url}\/fapi\/v1\/listenKey/)
        .to_return(status: 200, body: {}.to_json, headers: { "Content-Type" => "application/json" })
