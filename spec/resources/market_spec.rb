...
RSpec.describe BinanceUSDM::Resources::Market do
  ...
  describe '#ticker_24h' do
    context 'when symbol is provided' do
      it 'returns a Ticker model for the symbol' do
        stub_request(:get, %r{#{base_url}/fapi/v1/ticker/24hr\?.*symbol=BTCUSDT})
          .to_return(
            status: 200,
    ...
    context 'when no symbol is provided' do
      it 'returns an array of Ticker models' do
        stub_request(:get, %r{#{base_url}/fapi/v1/ticker/24hr})
          .to_return(
            status: 200,
  ...
  describe '#prices' do
    context 'when symbol is provided' do
      it 'returns price for the symbol' do
        stub_request(:get, %r{#{base_url}/fapi/v1/ticker/price\?.*symbol=BTCUSDT})
          .to_return(
            status: 200,
    ...
    context 'when no symbol is provided' do
      it 'returns prices for all symbols' do
        stub_request(:get, %r{#{base_url}/fapi/v1/ticker/price})
          .to_return(
            status: 200,
            body: [{ 'symbol' => 'BTCUSDT', 'price' => '50000.00' }].to_json,
            headers: { 'Content-Type' => 'application/json' }
  ...
  describe '#depth' do
    it 'returns order book depth' do
      stub_request(:get, %r{#{base_url}/fapi/v1/depth})
        .to_return(
          status: 200,
          body: { 'lastUpdateId' => 12_345, 'bids' => [['50000.00', '1.0']], 'asks' => [['50001.00', '2.0']] }.to_json,
          headers: { 'Content-Type' => 'application/json' }
        )
  ...
  describe '#trades' do
    it 'returns recent trades' do
      stub_request(:get, %r{#{base_url}/fapi/v1/trades})
        .to_return(
          status: 200,
  ...
  describe '#klines' do
    it 'returns kline/candlestick data' do
      stub_request(:get, %r{#{base_url}/fapi/v1/klines})
        .to_return(
          status: 200,
          body: [[1_699_000_000_000, '50000.00', '50500.00', '49900.00', '50200.00', '100.0']].to_json,
          headers: { 'Content-Type' => 'application/json' }
        )
  ...
  describe '#open_interest' do
    it 'returns open interest for symbol' do
      stub_request(:get, %r{#{base_url}/fapi/v1/openInterest})
        .to_return(
          status: 200,
  ...
  describe '#funding_rate_history' do
    it 'returns funding rate history' do
      stub_request(:get, %r{#{base_url}/fapi/v1/fundingRate})
        .to_return(
          status: 200,
          body: [{ 'symbol' => 'BTCUSDT', 'fundingRate' => '0.0001', 'fundingTime' => 1_699_000_000_000 }].to_json,
          headers: { 'Content-Type' => 'application/json' }
        )
